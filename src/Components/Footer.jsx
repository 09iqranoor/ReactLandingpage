  ...
  return (
    <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
      <div  className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
            <div className="flex-1 flex-col justify-start mr-10">
              <img src={logo} alt="logo" className="w-[266px] h-[72px] object-contain" />
                  <p className={`${styles.paragraph} mt-4 max-w-[310px] `}>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Totam iste i
                    nventore obcaecati voluptates! Cupiditate, esse!</p>
            </div>
      ...
      <div className="w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3f3r45]">
        <p className='font-poppins font-normal text-[18px] leading-[27px] text-white'>Lorem ipsum dolor sit amet consectetur.</p>
     
     <div className="flex flex-row md:mt-0 mt-6 ">

      {socialMedia.map((social , index ) =>{
           <img  key={social.id} src={social.icon} alt={social.id} className={`w-[120px] h-[21px] object-contain cursor-pointer  ${index !== socialMedia.length ? 'mr-6' : 'mr-0'} `} />
      })}
